...
async function requestScreenshot(options = {}) {
  try {
    ...
    if (settingsResponse && settingsResponse.isGloballyEnabled) {
      console.log(
        `Extension is globally enabled. Sending takeScreenshot request (options: ${JSON.stringify(options)})...`
      );
      chrome.runtime.sendMessage(
        { type: 'takeScreenshot', options: options },
        (response) => {
          if (chrome.runtime.lastError) {
            console.error(
              'Error sending takeScreenshot message:',
      ...
      console.log(
        `Extension is globally disabled. Screenshot request skipped.`
      );
    }
  } catch (error) {
...
function isElementOrAncestorInteractive(element, levels = 3) {
  ...
  for (
    ...
    try {
      ...
      console.warn(
        '[InteractiveCheck] Could not get computed style for element:',
        current,
        e
      );
    }
    ...
    console.log(
      `[InteractiveCheck] Checking level ${i}, Element: <${tagName}>`,
      {
        role,
        onclick: onclickAttr,
        isEditable,
    ...
    if (interactiveTags.includes(tagName)) {
      ...
      return {
        interactive: true,
        reason: `Interactive Tag (${tagName}) found in hierarchy`,
      };
    }
    // チェック2: Role属性
    if (role && interactiveRoles.includes(role)) {
      ...
      return {
        interactive: true,
        reason: `Interactive Role (${role}) found in hierarchy`,
      };
    }
    // チェック3: onclick属性
    if (onclickAttr) {
      ...
      return {
        interactive: true,
        reason: 'onclick attribute found in hierarchy',
      };
    }
    ...
    if (isEditable) {
      ...
      return {
        interactive: true,
        reason: 'contentEditable element found in hierarchy',
      };
    }
    // チェック5: Cursorスタイル
    if (cursorStyle === 'pointer') {
      ...
      return {
        interactive: true,
        reason: 'Cursor Style (pointer) found in hierarchy',
      };
    }
    ...
    if (classList && classList.contains('cursor-pointer')) {
      ...
      return {
        interactive: true,
        reason: 'cursor-pointer class found in hierarchy',
      };
    }
    ...
    if (['SVG', 'PATH', 'G'].includes(tagName)) {
      console.log(
        '[InteractiveCheck] Element is SVG/PATH/G, continuing to parent.'
      );
      // Keep checking parents
    } else if (!['DIV', 'SPAN', 'I', 'IMG'].includes(tagName)) {
      console.log(
        `[InteractiveCheck] Element <${tagName}> is not a common container, stopping ancestor check.`
      );
  ...
  console.log(
    '[InteractiveCheck] Result: FALSE (No interactive criteria met in hierarchy)'
  );
  return {
    interactive: false,
    reason: 'No interactive criteria met in hierarchy',
  };
}
...
document.addEventListener(
  ...
  async (event) => {
    ...
    if (interactive) {
      console.log(
        `Click event detected on interactive element or ancestor (Reason: ${reason}):`,
        target
      );
...
document.addEventListener('keydown', async (event) => {
  if (event.key === 'Enter') {
    ...
    const isEnterTarget =
      ...
      (role &&
        ['button', 'link', 'menuitem', 'checkbox', 'radio', 'option'].includes(
          role
        )) ||
